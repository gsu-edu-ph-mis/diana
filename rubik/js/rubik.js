...
Rubik.prototype.init = function() {
	...
	document.getElementById('cube').appendChild(this._node);
	
	OZ.CSS3.set(document.getElementById('cube'), "perspective", "460px");
	OZ.CSS3.set(this._node, "transform-style", "preserve-3d");
	
	this._build();
	this._update();
	OZ.Event.add(document.getElementById('cube'), "mousedown touchstart", this._dragStart.bind(this));
...
Rubik.prototype.randomize = function() {
	...
	var cb = function() {
		...
		if (remain > 0) { 
			...
			// document.getElementById('cube').appendChild(this._help.a);
			// document.getElementById('cube').appendChild(this._help.b);
...
Rubik.prototype._dragStart = function(e) {
	...
	this._drag.ec.push(OZ.Event.add(document.getElementById('cube'), "mousemove touchmove", this._dragMove.bind(this)));
	this._drag.ec.push(OZ.Event.add(document.getElementById('cube'), "mouseup touchend", this._dragEnd.bind(this)));
}
...
Rubik.prototype._rotateCubes = function(cubes, rotation) {
	...
	if (prefix === null) {
		...
		var e = OZ.Event.add(document.getElementById('cube'), "webkitTransitionEnd transitionend MSTransitionEnd oTransitionEnd", cb.bind(this));
	...
	for (var z=0;z<Rubik.SIZE;z++) {
		for (var y=0;y<Rubik.SIZE;y++) {
			for (var x=0;x<Rubik.SIZE;x++) {
				...
				if (z == 0) { cube.setFace(Face.FRONT, "#ff3000"); } // Red
				if (z == 2) { cube.setFace(Face.BACK, "#115ca0"); } // Blue
				
				if (x == 0) { cube.setFace(Face.LEFT, "#67ae36"); }
				if (x == 2) { cube.setFace(Face.RIGHT, "#ffc600"); } // yellow
				
				if (y == 0) { cube.setFace(Face.TOP, "#f35824"); }
				if (y == 2) { cube.setFace(Face.BOTTOM, "#fff"); }
